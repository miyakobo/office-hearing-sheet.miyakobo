...
- 01の先頭に**「ご依頼主区分」**（お客様／協力業者様／社内、ラジオ選択）を新設し、
  会社名・メールアドレス・お電話番号とあわせて**必須項目**にした（画面側でNext/送信をブロックし、
  Apps Script側でも同じ4項目を検証してから記録するダブルチェック）
- `gas/Code.gs`の`SUBMISSIONS_FIELDS`に`requesterType`（「ご依頼主区分」列）を追加、
  通知メール本文・ひとことまとめにも反映
- `admin.html`の一覧・詳細に「ご依頼主区分」列を追加

これにより、「送信元区分」（トークンによるサーバー側の会社判定、なりすまし不可）と
「ご依頼主区分」（本人が選ぶお客様／協力業者様／社内の立場、必須入力）の2軸で
送信者を把握できるようになっている。
